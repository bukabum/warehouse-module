...
function BulkIn() {
    ...
    const bulkIn = async (event) => {
        event.preventDefault();
        console.log(inputList)
        try {
            await axiosInstance.post('bulk/in/', {
                inputList: inputList,
            }).then(() => {
                setInputList([{ Amount: "", Reason: "", sparePK: "" }])
                setSnack(true)
                setSnackSeverity("success")
                setSnackText("Berhasil Menambah Stock Barang")
            })
        } catch(error){
            setSnack(true)
            setSnackSeverity("error")
            setSnackText("Gagal Menambah Stock Barang")
    }  
    }
    useEffect (() => {
        getCustomer();
    ...
    return (
        ...
        <h1>Stock Masuk</h1>
        <p>Menambahkan Jumlah Stock Item Secara Bersamaan</p>
        <form onSubmit={bulkIn}>
        {inputList.map((x, i) => {
        return (
