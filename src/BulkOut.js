...
function BulkOut() {
    const [inputList, setInputList] = useState([{ Amount: "", Reason: "", sparePK: "" }]);
    ...
    const handleAddClick = () => {
        setInputList([...inputList, { Amount: "", Reason: "", sparePK: "" }]);
        };
    ...
    const BulkOut = async (event) => {
        event.preventDefault();
        console.log(inputList)
        try {
            await axiosInstance.post('bulk/out/', {
                inputList: inputList,
            }).then(() => {
                setInputList([{ Amount: "", Reason: "", sparePK: "" }])
                setSnack(true)
                setSnackSeverity("success")
                setSnackText("Berhasil Mengurangi Stock Barang")
            })
        } catch(error){
            setSnack(true)
            setSnackSeverity("error")
            setSnackText("Gagal Mengurangi Stock Barang")
    }  
    }
    useEffect (() => {
        getCustomer();
    ...
    return (
        ...
        <h1>Stock Keluar</h1>
        <p>Mengurangi Jumlah Stock Item Secara Bersamaan</p>
        <form onSubmit={BulkOut}>
        {inputList.map((x, i) => {
        return (
